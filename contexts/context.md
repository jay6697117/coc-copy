...
- **项目名称**：COC Clone（部落冲突克隆版）
- **游戏类型**：策略 / 模拟经营 / 塔防 / 即时战略
- **目标平台**：Web（浏览器端）
- **引擎/技术栈**：Vite 7.x + TypeScript 5.7+ + PixiJS 8.17.x
- **美术方案**：Nano Banana 2（AI 生成）
- **当前阶段**：系统设计 ← [概念 | 系统设计 | 技术搭建 | 预生产 | 生产 | 打磨 | 发布]
...
> 由 `/brainstorm` 工作流于 2026-03-23 生成。支柱是项目的"宪法"，所有设计决策必须符合。

1. **建造即表达** — 基地布局是玩家个性的体现
2. **碎片化参与** — 每次打开只需 3-5 分钟就有事做
3. **长线成长感** — 从破旧村庄到坚不可摧的要塞
4. **社交粘性** — 部落系统让玩家有归属感

### 反面支柱（Anti-Pillars）
- **不做实时 PvP 对战** — 异步攻击是核心体验，实时对战开发成本太高
- **不做复杂叙事/剧情** — COC 的核心是系统驱动
- **不做 Pay-to-Win** — 宝石加速可以，但不能花钱才能赢
